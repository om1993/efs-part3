...
class MutualfundController extends Controller
{
    ...
    {
        // $imutualfund=mutualfund::where('id', $id)->select('customer_id','type','description','acquired_value','attained_date','recent_value','recent_date')->first();
        $mutualfund = mutualfund::where('cust_number', $id)->select('cust_number', 'type', 'acquired_value', 'attained_date', 'recent_value', 'recent_date')->first();

        $mutualfund = $mutualfund->toArray();
        return response()->json($mutualfund);
    }
